...
Upon finishing all that it's meant to do, a process becomes a zombie process. This is because although it's done all it's been programmed to do, it's still waiting faithfully for its parent to come and figure out how exactly it died. Once a process's parent executes \texttt{wait} or a variant of it, the zombie process is \textit{reaped}.\newline

The problem arises when you write (for lack of a better word) irresponsible parent processes. If your parents never end up calling wait() an appropriate amount of times and reaping their dead children, you'll end up having zombie processes just sitting around on your system. This phenomenon is known as a \textit{resource leak}.\newline

If all else fails, and your parent process simply cannot look after its own children- you're stuck. You'll have to wait till the parent process exits (hopefully gracefully, or the cycle repeats itself), and in that case, the \texttt{init} process will adopt those zombie children. The \texttt{init} process has a special ability to frequently check for (and clean up) zombie processes, so they'll be taken care of from there.\newline

The moral of the story is to make sure of the following- when you're writing code that forks child processes, make sure you dispose of them properly after they terminate (especially if you're making many children in a loop, for example), or you'll have zombie processes! Formally, this issue is called a resource leak, and it's generally something you don't want.

\section{Week 12}

More process control this week- but let's focus on zombie processes, reaping children, waiting, and the \texttt{exec} system call.
